# 更新履歴
 ...
 - master/HEAD
   * Nightly 56.0a1対応
   * ピン留めされたタブのラベルを表示しないようにした
   * タブが1つだけのウィンドウにおける最後のタブに関する設定の説明をより分かりやすくした
   * Suspend Tabの最新版との互換性を向上
   * ページをスクロールしなくてもタブを選択しただけで常にタブが既読になってしまっていたのを修正
   * タブのクローズボックスの手前に表示するようにしたサムネイルをツリー型タブの「折り畳まれたタブの数」のカウンターの後に表示するようにした
 - 0.5.2016010501
